-- Arkadaş davet kodu: kullanıcı başına benzersiz kod, tek seferlik kullanım, +10 güven puanı
...
set invite_code = public.generate_friend_invite_code()
where invite_code is null;

alter table public.profiles
  alter column invite_code set default public.generate_friend_invite_code();
...
revoke select (invite_code) on public.profiles from anon;
...
create table public.friend_invite_redemptions (
  ...
  unique (invitee_id),
  unique (inviter_id),
  constraint friend_invite_redemptions_no_self check (inviter_id <> invitee_id)
);

create index friend_invite_redemptions_inviter_idx
  on public.friend_invite_redemptions (inviter_id, created_at desc);

alter table public.friend_invite_redemptions enable row level security;

create policy friend_invite_redemptions_self_read on public.friend_invite_redemptions
...
create or replace function public.get_own_friend_invite_status()
returns table (
  invite_code text,
...
as $$
  select
    p.invite_code,
    exists (
      select 1 from public.friend_invite_redemptions r where r.invitee_id = p.id
    ) as has_redeemed,
    exists (
      select 1 from public.friend_invite_redemptions r where r.inviter_id = p.id
    ) as referral_reward_used
...
grant execute on function public.get_own_friend_invite_status() to authenticated;

-- Platform destekçisi: profil kolonu + user_badges senkronizasyonu + sohbet detayı

alter table public.profiles
  add column if not exists is_platform_supporter boolean not null default false;

create or replace function public.sync_platform_supporter_status(p_user_id uuid)
returns void
language plpgsql
security definer
...
begin
  ...
  set is_platform_supporter = v_active, updated_at = now()
  where id = p_user_id;
end;
$$;

create or replace function public.on_platform_supporter_badge_change()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  if tg_op = 'DELETE' then
    perform public.sync_platform_supporter_status(old.user_id);
    return old;
  end if;

  if new.badge_type = 'platform_supporter'
     or (tg_op = 'UPDATE' and old.badge_type = 'platform_supporter') then
    perform public.sync_platform_supporter_status(coalesce(new.user_id, old.user_id));
  end if;
...
drop trigger if exists user_badges_platform_supporter_sync on public.user_badges;
create trigger user_badges_platform_supporter_sync
  after insert or update or delete on public.user_badges
  for each row
  execute function public.on_platform_supporter_badge_change();

-- Mevcut destekçileri backfill et
update public.profiles p
set is_platform_supporter = true
where exists (
  select 1 from public.user_badges b
  where b.user_id = p.id and b.badge_type = 'platform_supporter'
);

-- Sohbet detayına destekçi alanı ekle
...
returns table (
  ...
  other_is_platform_supporter boolean,
...
as $$
  select
    ...
    other_p.is_platform_supporter as other_is_platform_supporter,

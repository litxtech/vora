-- Otomatik push: tek seferlik tarih/saat planlama (trigger_type = scheduled)

alter table public.push_automation_templates
  drop constraint if exists push_automation_templates_trigger_type_check;

alter table public.push_automation_templates
  add constraint push_automation_templates_trigger_type_check
  check (trigger_type in ('feed_activity', 'interval', 'manual', 'scheduled'));

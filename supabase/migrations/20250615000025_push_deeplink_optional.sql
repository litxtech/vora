-- Push şablonları: yönlendirme isteğe bağlı (boş = tıklanınca yalnızca bildirim kutusu)

alter table public.push_automation_templates
  alter column deep_link drop not null;

alter table public.push_automation_templates
  alter column deep_link drop default;
...
begin
  ...
  v_trigger := coalesce(nullif(p_payload->>'trigger_type', ''), 'manual');
  v_next_run := nullif(p_payload->>'next_run_at', '')::timestamptz;
  v_deep_link := nullif(trim(coalesce(p_payload->>'deep_link', '')), '');
  ...
  if v_id is null then
    ...
    values (
      ...
      v_trigger,
      coalesce(nullif(p_payload->>'event_type', ''), 'system')::public.notification_event_type,
      ...
      coalesce(nullif(p_payload->>'pref_key', ''), 'system'),
    ...
    set
      ...
      deep_link = case
        when p_payload ? 'deep_link' then v_deep_link
        else deep_link
      end,
      region_ids = case
        when p_payload->'region_ids' is null or jsonb_typeof(p_payload->'region_ids') <> 'array'

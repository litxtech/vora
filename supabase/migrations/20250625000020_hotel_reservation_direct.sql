-- Otel rezervasyonu: Stripe olmadan doğrudan "Rezerve Et"
...
begin
  ...
  select coalesce(username, full_name, 'Misafir') into v_guest_name from public.profiles where id = v_guest_id;

  v_receipt_body := format(
    'Tahmini %s ₺ · %s → %s · %s gece · %s kişi · ödeme otelde',
  ...
  perform public.log_hotel_reservation_event(
    ...
    jsonb_build_object('gross_cents', v_gross, 'payment_status', 'at_hotel')
  );
...
grant execute on function public.create_hotel_reservation to authenticated;

-- Öncü: kolon, senkronizasyon, admin RPC ve mesajlaşma detayı

alter table public.profiles
  add column if not exists is_pioneer boolean not null default false;

create or replace function public.sync_pioneer_status(p_user_id uuid)
returns void
language plpgsql
security definer
...
begin
  select exists (
    ...
    where user_id = p_user_id and badge_type = 'pioneer'
  ) into v_active;

  update public.profiles
  set is_pioneer = v_active, updated_at = now()
  where id = p_user_id;
end;
$$;

create or replace function public.on_pioneer_badge_change()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  if tg_op = 'DELETE' then
    perform public.sync_pioneer_status(old.user_id);
    return old;
  end if;

  if new.badge_type = 'pioneer' or (tg_op = 'UPDATE' and old.badge_type = 'pioneer') then
    perform public.sync_pioneer_status(coalesce(new.user_id, old.user_id));
  end if;
...
drop trigger if exists user_badges_pioneer_sync on public.user_badges;
create trigger user_badges_pioneer_sync
  after insert or update or delete on public.user_badges
  for each row
  execute function public.on_pioneer_badge_change();

update public.profiles p
set is_pioneer = true
where exists (
  select 1 from public.user_badges b
  where b.user_id = p.id and b.badge_type = 'pioneer'
);

create or replace function public.admin_grant_pioneer(p_user_id uuid)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  values (p_user_id, 'pioneer', now())
  on conflict (user_id, badge_type) do nothing;

  perform public.sync_pioneer_status(p_user_id);

  perform public.notify_profile_user(
    p_user_id,
    'badge_earned',
    'Öncü',
    'Tebrikler! Vora platformunun öncü rozeti size verildi.',
    jsonb_build_object('badgeType', 'pioneer')
  );
end;
$$;

create or replace function public.admin_revoke_pioneer(p_user_id uuid)
returns void
language plpgsql
security definer
set search_path = public
...
grant execute on function public.admin_grant_pioneer(uuid) to authenticated;
grant execute on function public.admin_revoke_pioneer(uuid) to authenticated;
...
as $$
  select
    ...
    other_p.is_pioneer as other_is_pioneer,

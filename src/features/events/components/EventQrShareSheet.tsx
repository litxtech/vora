...
import { EventQrShareCard, type EventQrShareCardProps } from '@/features/events/components/EventQrShareCard';
import {
  captureEventQrShareCard,
  saveEventQrShareCardToGallery,
  shareEventQrLinkOnly,
  shareEventQrShareCardImage,
  shareEventQrWhatsApp,
} from '@/features/events/services/eventQrShare';
import { radius, spacing } from '@/constants/theme';
import { useTheme } from '@/providers/ThemeProvider';

type EventQrShareSheetProps = EventQrShareCardProps & {
  visible: boolean;
  onClose: () => void;
};
...
export function EventQrShareSheet({
  visible,
  onClose,
  token,
  title,
  ...
  const payload = { token, title, eventId, startsAt, locationName };

  useEffect(() => {
    if (!visible) return;
    setCardReady(false);
    dragY.value = 0;
  }, [visible, token, eventId, dragY]);
  ...
  const runWithCapture = useCallback(
    async (
      ...
      setBusy(true);
      try {
        const captured = await captureEventQrShareCard(cardRef);
        if (!captured.uri) {
  ...
  const actions: ShareAction[] = [
    {
      ...
      subtitle: 'Vora giriş kartını fotoğraf olarak kaydet',
      icon: 'download-outline',
      color: CARD_ACCENT,
      run: async () => {
        await runWithCapture(saveEventQrShareCardToGallery, {
          successMessage: 'Kart galerinize kaydedildi.',
        });
      },
    },
    {
      ...
      color: '#5C6BC0',
      run: async () => {
        await runWithCapture(shareEventQrShareCardImage);
      },
    },
    {
      id: 'whatsapp',
      label: 'WhatsApp',
      subtitle: 'Metin ve giriş bağlantısı',
      icon: 'logo-whatsapp',
      color: '#25D366',
      run: async () => {
        const result = await shareEventQrWhatsApp(payload);
    ...
    },
    {
      ...
      run: async () => {
        await shareEventQrLinkOnly(payload);
        onClose();
      },
    },
  ];
      ...
      <View style={[styles.root, { backgroundColor: colors.overlay }]}>
        ...
        >
          <GestureDetector gesture={dismissPan}>
            <View style={styles.dragZone}>
              ...
              <Text secondary variant="caption" style={styles.subtitle}>
                QR kodunu Vora kartıyla dışarı aktar
              </Text>
            </View>
          </GestureDetector>
          ...
          {actions.map((action) => (
            <Pressable
              key={action.id}
              disabled={busy || (action.id !== 'link' && action.id !== 'whatsapp' && !cardReady)}
              onPress={() => action.run()}
              style={({ pressed }) => [
                styles.row,

...
import { fetchPersonnelCenterStats,
...
import type { JobListingDetail, ListingApplicationStats } from '@/features/personnel-center/types';
...
function formatSalary(listing: JobListingDetail): string {
  if (listing.salaryType === 'negotiable') return 'Görüşülecek';
  if (listing.salaryRange) return listing.salaryRange;
  const label = SALARY_TYPE_OPTIONS.find((o) => o.value === listing.salaryType)?.label;
  return label ?? 'Belirtilmedi';
}
...
export function JobDetailScreen() {
  ...
  const { applyTarget, openApplySheet, closeApplySheet, submitApplication } = usePersonnelApply(user?.id, {
    onSubmitted: () => {
      if (id) fetchListingApplicationStats('job', id).then(setApplicationStats);
    },
  });
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    setError(null);
    const data = await fetchJobListingDetail(id);
    if (!data || data.status === 'removed') {
      setError('İlan bulunamadı veya yayından kaldırıldı.');
      setListing(null);
    } else {
      setListing(data);
      fetchPersonnelCenterStats(data.regionId).then(setCenterStats);
      fetchListingApplicationStats('job', data.id).then(setApplicationStats);
    }
    setLoading(false);
  }, [id]);
  ...
  const salaryLabel = listing ? formatSalary(listing) : '';
  const workplacePhotos = listing?.workplaceMediaUrls ?? [];
  const coverPhoto = workplacePhotos[0] ?? null;

  const openPhotoViewer = (index: number) => {
    if (!workplacePhotos.length) return;
  ...
  const handleShare = async () => {
    if (!listing) return;
    await Share.share({
      message: `${listing.title}\n${salaryLabel} · ${jobTypeLabel(listing.jobType)}\n\nVora uygulamasında iş ilanını görüntüle.`,
      title: listing.title,
    });
  };
  ...
  const handleApply = async () => {
    ...
    openApplySheet('job', id, listing.title);
  };
  ...
  const handleMessage = async () => {
    ...
    const result = await sharePersonnelListingInChat(
      ...
      listing.authorId,
      user.id,
      'Merhaba, iş ilanınız hakkında bilgi almak istiyorum.',
    );
    if (result.error) {
  ...
  const handleEdit = () => {
    if (!listing) return;
    router.push(jobEditPath(listing.id) as never);
  };

  const handleFill = () => {
    if (!listing || !user?.id) return;
    Alert.alert('Pozisyon Doldu', 'İlan kapatılacak ve başvuranlar bilgilendirilecek.', [
      { text: 'Vazgeç', style: 'cancel' },
      {
        text: 'Kapat',
        onPress: async () => {
          setFilling(true);
          const result = await fillJobListing(listing.id);
          setFilling(false);
  ...
  const handleRemove = () => {
    if (!listing || !user?.id) return;
    Alert.alert('İlanı Kaldır', 'İş ilanı yayından kaldırılacak.', [
      { text: 'Vazgeç', style: 'cancel' },
      {
        text: 'Kaldır',
        style: 'destructive',
        onPress: async () => {
          setRemoving(true);
          const result = await removeJobListing(listing.id, user.id);
          setRemoving(false);
          if (result.error) Alert.alert('Hata', result.error);
          else {
    ...
    <GradientBackground>
      ...
      >
        <View style={styles.heroWrap}>
          {coverPhoto ? (
            <>
              <Pressable onPress={() => openPhotoViewer(0)} style={styles.heroImageWrap} accessibilityRole="imagebutton" accessibilityLabel="İşyeri fotoğrafını büyüt">
                <Image source={{ uri: coverPhoto }} style={[styles.heroImage, { height: 280 + insets.top }]} />
                <LinearGradient
                  colors={['rgba(0,0,0,0.5)', 'transparent', 'rgba(0,0,0,0.08)']}
                  locations={[0, 0.35, 1]}
                ...
                <View style={[styles.heroTopBar, { top: insets.top + spacing.xs }]}>
                  ...
                  <View style={styles.heroTopActions}>
                    ...
                    {!isOwner && showDetailReport ? (
                      ...
                      <Pressable onPress={handleRemove} disabled={removing} style={[styles.iconBtn, { backgroundColor: 'rgba(0,0,0,0.45)' }]}>
                        <Ionicons name="trash-outline" size={20} color={colors.danger} />
                      </Pressable>
                    ) : null}
                  </View>
                </View>
                {workplacePhotos.length > 1 ? (
                  <View style={[styles.photoCountPill, { bottom: spacing.md }]}>
                    <Ionicons name="images-outline" size={12} color="#fff" />
                    <Text variant="caption" style={styles.photoCountText}>
                      {workplacePhotos.length} fotoğraf
                    </Text>
                  </View>
                ) : null}
              </Pressable>
              ...
              <View style={[styles.heroContentBelow, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                <View style={styles.heroBadges}>
                  {listing.isUrgent ? (
                    <View style={[styles.badgeMuted, { backgroundColor: `${colors.danger}14` }]}>
                  ...
                  <View style={[styles.badgeMuted, { backgroundColor: `${PERSONNEL_ACCENT}14` }]}>
                    <Text variant="caption" style={{ color: PERSONNEL_ACCENT, fontWeight: '700' }}>
                      {jobTypeLabel(listing.jobType)}
                    </Text>
                  </View>
                </View>
                <Text variant="h2" style={[styles.heroTitleBelow, { color: colors.text }]}>
                  {listing.title}
                </Text>
                ...
                <View style={styles.salaryBlock}>
                  ...
                  <Text variant="h3" style={[styles.salaryValue, { color: colors.text }]}>
                    {salaryLabel}
                  </Text>
                </View>
              </View>
            ...
            <>
              <LinearGradient
                ...
                style={[styles.heroPlaceholder, { height: 240 + insets.top, paddingTop: insets.top }]}
              >
                <Ionicons name="briefcase" size={72} color={`${PERSONNEL_ACCENT}55`} />
              </LinearGradient>
              <LinearGradient
              ...
              <View style={styles.heroContent}>
                ...
                <Text variant="h3" style={{ color: '#fff', fontWeight: '800' }}>
                  {salaryLabel}
                </Text>
              </View>
            </>
        ...
        <View style={styles.content}>
          <View style={styles.statsRow}>
            <View style={[styles.statCell, { backgroundColor: `${colors.surface}AA`, borderColor: colors.border }]}>
              <Ionicons name="eye-outline" size={14} color={PERSONNEL_ACCENT} />
          ...
          {!isOwner ? (
            <GlassCard style={[styles.section, { borderColor: `${PERSONNEL_ACCENT}22` }]}>
          ...
          {workplacePhotos.length > 1 ? (
            <GlassCard style={[styles.section, styles.galleryCard, { borderColor: `${PERSONNEL_ACCENT}22` }]}>
              <View style={styles.galleryHeader}>
                <View style={[styles.galleryHeaderIcon, { backgroundColor: `${PERSONNEL_ACCENT}14` }]}>
                  <Ionicons name="images-outline" size={18} color={PERSONNEL_ACCENT} />
                </View>
                <View style={styles.galleryHeaderText}>
                  <Text variant="label">İşyeri görselleri</Text>
                  <Text secondary variant="caption">
                    {workplacePhotos.length} fotoğraf
                  </Text>
                </View>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.galleryRow}>
                {workplacePhotos.map((uri, index) => (
                  <Pressable
                    key={`${uri}-${index}`}
                    onPress={() => openPhotoViewer(index)}
                    style={({ pressed }) => [styles.galleryTile, { opacity: pressed ? 0.88 : 1 }]}
                    accessibilityRole="button"
                    accessibilityLabel={`İşyeri görseli ${index + 1}`}
                  >
                    ...
                    <Text
                      ...
                      style={[
                        ...
                        { color: index === 0 ? PERSONNEL_ACCENT : colors.textMuted, fontWeight: index === 0 ? '700' : '500' },
                      ]}
                    >
                      {index === 0 ? 'Kapak' : `Görsel ${index + 1}`}
                    </Text>
                  </Pressable>
                ))}
              </ScrollView>
          ...
          <GlassCard style={styles.section}>
            <Text variant="label">İş tanımı</Text>
          ...
          <View style={styles.tiles}>
            ...
            {listing.experienceRequired ? (
              <InfoTile icon="ribbon-outline" label="Deneyim" value={listing.experienceRequired} accent={PERSONNEL_ACCENT} />
            ) : null}
          ...
          {isFilled ? (
            <GlassCard style={[styles.filledBanner, { borderColor: `${colors.success}44`, backgroundColor: `${colors.success}10` }]}>
              ...
              <Text variant="caption" style={{ color: colors.success, fontWeight: '700' }}>
                Bu pozisyon doldu — yeni başvuru kabul edilmiyor.
              </Text>
            </GlassCard>
          ) : null}
          ...
          {isOwner ? (
            <>
              <ListingOwnerStatsCard listingType="job" listingId={listing.id} />
      ...
      {showDetailReport && id ? (
        <ReportSheet visible={showReport} targetType="job_listing" targetId={id} onClose={() => setShowReport(false)} />
      ) : null}
...
const styles = StyleSheet.create({
  ...
  photoCountText: { color: '#fff', fontWeight: '700' },
  heroContentBelow: {
  ...
  heroTitleBelow: { fontWeight: '800' },
  salaryBlock: { gap: 2, marginTop: spacing.xs },
  salaryValue: { fontWeight: '800' },
  heroPlaceholder: {
    height: 240,
    alignItems: 'center',
  ...
  galleryHeaderText: { flex: 1, gap: 2 },
  galleryRow: { gap: spacing.md, paddingRight: spacing.sm },
  galleryTile: {
    width: 128,
    gap: spacing.xs,
    alignItems: 'center',
